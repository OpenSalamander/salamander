...
// Patched version based on pnglite-0.1.17.zip
// Added support for indexed-color PNGs
...
static int png_read_trns(png_t* png, unsigned length) 
{
	char *chunk;
	...
	calc_crc = crc32(calc_crc, (unsigned char*)"tRNS", 4);
...
static int png_read_plte(png_t* png, unsigned length) 
{
	char *chunk;
	int result;
...
#if DO_CRC_CHECKS
	calc_crc = crc32(0L, Z_NULL, 0);
	calc_crc = crc32(calc_crc, (unsigned char*)"PLTE", 4);
	calc_crc = crc32(calc_crc, (unsigned char*)chunk, length);
...
static int png_process_chunk(png_t* png)
{
	...
	if(type == *(unsigned int*)"tRNS")
	{
		png->transparencylen = length;
		png->transparency = png_alloc(png->transparencylen);

		if(!png->transparency)
			return PNG_MEMORY_ERROR;

		png_read_trns(png, length);
	}
	else
	if(type == *(unsigned int*)"PLTE")
	{
		png->palettelen = length;
		png->palette = png_alloc(png->palettelen);

		if(!png->palette)
			return PNG_MEMORY_ERROR;

		png_read_plte(png, length);
	}
	else
