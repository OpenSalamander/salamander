...
extern BOOL noise_section_initialized;  // Petr Solin: AppVerifier has found that noise_section is initialized twice, this bool solves the problem
#endif MPEXT
...
void random_ref(void)
{
    if (!random_active) {
#ifdef MPEXT
        if (!noise_section_initialized) // Petr Solin: AppVerifier has found that noise_section is initialized twice, this bool solves the problem
        {
