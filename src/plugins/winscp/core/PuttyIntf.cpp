...
BOOL noise_section_initialized = FALSE;  // Petr Solin: AppVerifier has found that noise_section is initialized twice, this bool solves the problem
bool SaveRandomSeed;
char appname_[50];
...
void __fastcall PuttyInitialize()
{
  ...
  if (!noise_section_initialized)
  {
    InitializeCriticalSection(&noise_section);
    noise_section_initialized = TRUE;
  }
...
void __fastcall PuttyFinalize()
{
  ...
  if (noise_section_initialized)
  {
    ...
    noise_section_initialized = FALSE;  // Petr Solin: AppVerifier has found that noise_section is initialized twice, this bool solves the problem
  }
}
//---------------------------------------------------------------------------
