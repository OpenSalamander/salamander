  ...
  for (UInt32 i = 0;; lps->OutSize += curUnpacked, lps->InSize += curPacked)
  {
    ...
    try
    {
      ...
      if (result == S_FALSE || result == E_NOTIMPL)
      {
        ...
        // JRY FIX: exit with error when incorrect password is used
        // Otherwise Salamander doesn't report any error and can delete source 7z archive (an Unpack dialog option).
        // Note: it looks like this problem is solved in 7-Zip 19.0 source code.
        return result;
      }
