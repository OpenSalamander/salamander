...
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE HashThreadFunc2Wrap(void *p) {
  return (THREAD_FUNC_RET_TYPE) RunThreadWithCallStackObject((LPTHREAD_START_ROUTINE) HashThreadFunc2, p);
}
...
// OPENSAL_7ZIP_PATCH BEGIN
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE BtThreadFunc2Wrap(void *p) {
  return (THREAD_FUNC_RET_TYPE) RunThreadWithCallStackObject((LPTHREAD_START_ROUTINE) BtThreadFunc2, p);
}
// OPENSAL_7ZIP_PATCH END
...
// OPENSAL_7ZIP_PATCH BEGIN
  RINOK(MtSync_Create(&p->hashSync, HashThreadFunc2Wrap, p, kMtHashNumBlocks));
  RINOK(MtSync_Create(&p->btSync, BtThreadFunc2Wrap, p, kMtBtNumBlocks));
// OPENSAL_7ZIP_PATCH END
  return SZ_OK;
}
