                                    ...
                                    Int64* len)
{
    ...
    if (block < h->reset_table.block_count - 1)
    {
        ...
        if (_chm_fetch_bytes(h, buffer,
                             (UInt64)h->data_offset + (UInt64)h->rt_unit.start + (UInt64)h->reset_table.table_offset + (UInt64)block * 8,
                             remain) != remain ||
        ...
        remain = 8;
        if (_chm_fetch_bytes(h, buffer,
                             (UInt64)h->data_offset + (UInt64)h->rt_unit.start + (UInt64)h->reset_table.table_offset + (UInt64)block * 8 + 8,
                             remain) != remain ||
                                   ...
                                   UChar** ubuffer)
{
    ...
    if (blockAlign != 0)
    {
        ...
        for (i = blockAlign; i > 0; i--)
        {
            ...
            if (h->lzx_last_block != (Int32)curBlockIdx)
            {
                ...
                if (!_chm_get_cmpblock_bounds(h, curBlockIdx, &cmpStart, &cmpLen) ||
                    cmpLen < 0 ||
                    cmpLen > (Int64)h->reset_table.block_len + 6144 ||
