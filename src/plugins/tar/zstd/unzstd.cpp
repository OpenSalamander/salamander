...
CZStd::CZStd(const char *filename, HANDLE file, unsigned char *buffer, unsigned long start, unsigned long read, CQuadWord inputSize):
...
BOOL
CZStd::DecompressBlock(unsigned short needed)
{
  if (EndReached)
    return TRUE;
  size_t lastRet = 1;
