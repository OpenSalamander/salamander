...
// WARNING: TRACE_C must not be used in a DLL's DllMain, or in any code called from DllMain,
//        otherwise a deadlock will occur; see the implementation of
//        C__Trace::SendMessageToServer

// CALLSTK_MEASURETIMES macro - enables measurement of the time spent preparing call-stack messages (measured as a ratio to
//                              total function run time)
//                              WARNING: it must also be enabled separately for each plugin
// CALLSTK_DISABLEMEASURETIMES macro - suppresses measurement of the time spent preparing call-stack messages in DEBUG builds
...
public:
#if (defined(_DEBUG) || defined(CALLSTK_MEASURETIMES)) && !defined(CALLSTK_DISABLEMEASURETIMES)
    // 'doNotMeasureTimes'==TRUE = do not measure Push for this call-stack macro (it probably slows things down a lot, but
    // we do not want to remove it because it is too important for debugging)
#ifdef __BORLANDC__
...
// empty macro: tells CheckStk that we do not want a call-stack message for this function
...
// do not use this if SalamanderDebug->TraceAttachThread has already been called for this thread (directly
// or, for example, when the thread is started via CThreadQueue::StartThread)
