...
/* explode.c -- modified by Lucas Cerman 
   version 1.0b, February 2000
   
   based on file explode.c distributed with infozip,
   puted in the public domain by Mark Adler
...
/* routines here */
int get_tree(CDecompressionObject* decompress, unsigned* l, unsigned n);
int explode_lit(CDecompressionObject* decompress, struct huft* tb, struct huft* tl,
                struct huft* td, int bb, int bl, int bd,
                unsigned bdl);
int explode_nolit(CDecompressionObject* decompress, struct huft* tl, struct huft* td,
                  int bl, int bd, unsigned bdl);
int Explode(CDecompressionObject* decompress);
...
#define NEEDBITS(decomp, n) \
    { \
        ...
        { \
            b |= ((ulg)NextByte((decomp))) << k; \
            k += 8; \
            if ((decomp)->Input->Error && (decomp)->Input->Error != IDS_EOF) \
            { \
...
#define DECODEHUFT(decomp, htab, bits, mask) \
    { \
        NEEDBITS(decomp, (unsigned)(bits)) \
...
int get_tree(CDecompressionObject* decompress,
   ...
   Otherwise zero is returned. */
{
    ...
    /* get bit lengths */
    i = NextByte(decompress) + 1; /* length/count pairs to read */
    k = 0;                        /* next code */
    do
    {
        b = ((j = NextByte(decompress)) & 0xf) + 1; /* bits in code (1..16) */
...
int explode_lit(CDecompressionObject* decompress,
                struct huft* tb, /* literal, length, and distance tables */
                struct huft* tl,
                struct huft* td,
                int bb, int bl, int bd, /* number of bits decoded by those */
   ...
   window (of size 2^(6+bdl) bytes). */
{
    ...
    while (s > 0) /* do until ucsize bytes uncompressed */
    {
        ...
        if (b & 1) /* then literal--decode it */
        {
            DUMPBITS(1)
            s--;
            DECODEHUFT(decompress, tb, bb, mb) /* get coded literal */
        ...
        {
            DUMPBITS(1)
            NEEDBITS(decompress, bdl) /* get distance low bits */
            d = (unsigned)b & mdl;
            DUMPBITS(bdl)
...
int explode_nolit(CDecompressionObject* decompress,
                  struct huft* tl,
   ...
   bits are read in, uncoded, for the low distance bits. */
{
    ...
    bl = 7;
    bd = decompress->csize > 200000L ? 8 : 7;

    decompress->Input->BitCount = 0;
    decompress->Input->BitBuf = 0;

    if (decompress->Flag & 4)
    /* With literal tree--minimum match length is 3 */
    {
        bb = 9; /* base table size for literals */
        if ((r = get_tree(decompress, l, 256)) != 0)
            return (int)r;
        if ((r = huft_build(decompress, l, 256, 256, NULL, NULL, &tb, &bb)) != 0)
        {
            if (r == 1)
                huft_free(decompress, tb);
            return (int)r;
        }
        if ((r = get_tree(decompress, l, 64)) != 0)
        {
            huft_free(decompress, tb);
            return (int)r;
        }
        if ((r = huft_build(decompress, l, 64, 0, cplen3, extra, &tl, &bl)) != 0)
        {
            if (r == 1)
    ...
    /* No literal tree--minimum match length is 2 */
    {
        ...
        if ((r = get_tree(decompress, l, 64)) != 0)
            return (int)r;
        if ((r = huft_build(decompress, l, 64, 0, cplen2, extra, &tl, &bl)) != 0)
        {
            if (r == 1)
                huft_free(decompress, tl);
            return (int)r;
    ...
    {
        bdl = 6;
        r = huft_build(decompress, l, 64, 0, cpdist4, extra, &td, &bd);
    }
    if (r != 0)
    {
        if (r == 1)
    ...
    if (tb != NULL)
    {
        r = explode_lit(decompress, tb, tl, td, bb, bl, bd, bdl);
        huft_free(decompress, tb);
    }
    else
    {
        r = explode_nolit(decompress, tl, td, bl, bd, bdl);
    }
