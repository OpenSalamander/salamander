    ...
    ISzAlloc *allocTemp)
{
  ...
  while(1)
  {
    ...
    switch((int)type)
    {
      ...
      {
        Byte external;
        RINOK(SzReadBoolVector2(sd, (UInt32) numFiles, &timeDefined, allocMain->Alloc))

        RINOK(SzReadByte(sd, &external));
        if (external != 0) {
          UInt64 dataIndex;
          RINOK(SzReadNumber(sd, &dataIndex));
        }

        times = (FILETIME *)allocMain->Alloc(numFiles * sizeof(FILETIME));
        for (i = 0; i < numFiles; i++) {
          if (timeDefined[i]) {
            FILETIME fileTime;
            UInt32 low, high;
            RINOK(SzReadUInt32(sd, &low));
            RINOK(SzReadUInt32(sd, &high));
            fileTime.dwLowDateTime = low;
            fileTime.dwHighDateTime = high;
...

  {
    ...
    for(i = 0; i < numFiles; i++)
    {
      ...
      if (timeDefined[i]) {
        file->IsLastWriteTimeDefined = 1;
        file->LastWriteTime = times[i];
      }
      else
        file->IsLastWriteTimeDefined = 0;
      // end of DA
    }
  }
