@rem Export spellcheck data from all module .atp projects.
@rem Run from a language project directory.

@if not exist spellcheck @mkdir spellcheck
...
@echo.
@echo Spellcheck exported for all modules.
@echo.
@pause
@exit /b
...
:run_export
@echo Exporting spellcheck for %1 ...
@if not exist "%1" (
  @echo   Skipping %1 - project file not found.
  @exit /b
)
@call "%TRANSLATOR_EXE%" -build-dir "%BUILD_DIR%" -quiet-export-spellcheck "spellcheck" %1
