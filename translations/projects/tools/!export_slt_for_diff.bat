@rem Export SLT text archives for diff (without layout info).
...
@echo.
@echo SLT (for diff) exported for all modules.
@echo.
@pause
@exit /b
...
:run_export
@echo Exporting SLT (for diff) for %1 ...
@if not exist "%1" (
  @echo   Skipping %1 - project file not found.
  @exit /b
)
@call "%TRANSLATOR_EXE%" -build-dir "%BUILD_DIR%" -quiet-export-slt-for-diff "slt" %1
