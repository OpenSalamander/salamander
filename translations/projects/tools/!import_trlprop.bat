@rem Import translation properties from binary SLGs for all modules.
...
@echo.
@echo Translation properties imported for all modules.
@echo.
@pause
@exit /b
...
:run_import
@echo Importing trlprop for %1 ...
@if not exist "%1" (
  @echo   Skipping %1 - project file not found.
  @exit /b
)
@call "%TRANSLATOR_EXE%" -build-dir "%BUILD_DIR%" -quiet-import-trlprop %1
